...
Additional optional variables such as temperature standard deviation, precipitation standard deviation and a simple dryness index are also implemented, although they were not used in the current experiments. This mode was used in all of the test runs shown in this report.
...
The first direction is to investigate different representations of the climate data. In this work, every location is described using 31 climate variables. Other feature representations, such as dimensionality reduction techniques, seasonal summaries or standardized climate variables, may produce different regionalizations and should be investigated.

Another important direction is the development of alternative fitness functions. The current fitness function is largely based on centroid compactness and spatial coherence. Since the centroid-based algorithms consistently achieved the best results, it would be interesting to investigate whether this conclusion still holds when using fitness functions based on different clustering principles.
